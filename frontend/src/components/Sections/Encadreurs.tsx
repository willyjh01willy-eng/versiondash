...
import { Search, Plus, Mail, Phone, Users, CreditCard as Edit, Trash2, MoreVertical, UserX, UserCheck } from 'lucide-react';
...
export default function Encadreurs() {
  ...
  const loadEncadreurs = async () => {
    try {
      ...
      console.log('Encadreurs data:', data);
      const encadreursWithCount = await Promise.all(
        data.map(async (enc) => {
          try {
            const count = await encadreurService.getEncadreurInternCount(enc.encadreurId);
  ...
  const handleSuspendAccount = async (encadreur: any, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const { userService } = await import('../../services/userService');
      await userService.suspendAccount(encadreur.id);
      loadEncadreurs();
      setOpenMenuId(null);
    } catch (error: any) {
  ...
  const handleActivateAccount = async (encadreur: any, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const { userService } = await import('../../services/userService');
      await userService.activateAccount(encadreur.id);
      loadEncadreurs();
      setOpenMenuId(null);
    } catch (error: any) {
                  ...
                  <tr key={encadreur.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="h-10 w-10 rounded-full bg-gradient-to-br from-orange-400 to-red-400 flex items-center justify-center text-white font-semibold">
                          <div className="h-10 w-10 rounded-full flex items-center justify-center bg-gradient-to-br from-orange-500 to-red-500 text-white font-semibold overflow-hidden">
                    {encadreur.avatar ? (
                      <img
                        src={encadreur.avatar}
                        alt={`${encadreur.prenom} ${encadreur.nom}`}
                        className="h-full w-full object-cover"
                      />
                    ) : (
                      <>
                        {encadreur.prenom[0].toUpperCase()}
                        {encadreur.nom[0].toUpperCase()}
                      </>
                    )}
                  </div>
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2 inline-flex text-xs leading-5 ">
                        {getStatusBadge(encadreur.accountStatus)}
                        ...
                        {authUser?.role === 'ADMIN' && (
                          <div className="relative">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                setOpenMenuId(openMenuId === encadreur.id ? null : encadreur.id);
                              }}
                              className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                            >
                              <MoreVertical className="h-4 w-4 text-gray-600 dark:text-gray-400" />
                            </button>
                            {openMenuId === encadreur.id && (
                              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-10">
                                {encadreur.accountStatus === 'ACTIVE' ? (
                                  <button
                                    onClick={(e) => handleSuspendAccount(encadreur, e)}
                                  ...
                                  <button
                                    onClick={(e) => handleActivateAccount(encadreur, e)}
